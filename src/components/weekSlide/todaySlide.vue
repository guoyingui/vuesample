                ...
                <template v-for="(item, index) in dates">
                    <div 
                        ...
                        <div class="day" v-for="(day, dayIndex) in getDaies(item.date)" :key="dayIndex" >
                            <div @click.stop="dayClickHandle(day.date)" :class="{today: day.isToday, sameDay: day.isDay && day.isToday, isSelected: day.isSelected, isError:day.isError, isRegular:day.isRegular, isWeek: day.isWeek}">{{day.week}}<br><strong>{{day.date.split('-')[2]}}</strong></div>
                        </div>
                    </div>
                </template>
            </div>
...
.week-slider{
    width: 100%; height: 40px; overflow: hidden; padding: 10px 0;
    .sliders{
        position: relative;
        .slider{
            height: 40px; width: 100%; display: flex; position: absolute; top: 0; left: 0; overflow: hidden;
            .day{
                flex: 1;
                div{
                    height: 28px; max-width: 40px; padding: 6px 0; margin: auto; text-align: center; line-height: 15px; font-size: 12px;
                    &.today{
                        border-radius: 50%;  
                    }
                    &.isSelected{
                        border-radius: 50%; background-color: #3399ff; color: #FFF !important;
                    }
                    &.sameDay{
                        border-radius: 50%; background-color: #3399ff; color: #FFF;
                    }
                    &.isError{
                      color: #ff3831;
                    }
